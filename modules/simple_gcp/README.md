...
module "simple_gcp" {
  ...
  machines = {
    1    = ["e2-micro", "sles4sap_15.4", 0, 0],
    2    = ["e2-micro", "sles4sap_15.5", 0, 0,
    "3a" = ["e2-micro", "sles4sap_15.6", 0, 0],
    "3b" = ["e2-micro", "sles4sap_15.7", 0, 0],
    4    = ["e2-micro", "sles4sap_15.7", 0, 0]
  }
  ...
  bastion = ["e2-micro", "sles4sap_12.5", 0, 0]
...
* `machines` (mandatory)
  ...
  **Take care, that the `key` is unique! Terraform will always take silently the last hit. "Renaming" of machines can lead to strange effects and might brake your environment!**
  ...
  The identifiers must be provided by the file `sizing_aws.yaml` in the project root directory, which must contain the identifiers you want to use, which point to the GCP instance types. 
  ...
  The identifiers and the images must be provided by the file `images_aws.yaml` in the project root directory, which must contain the identifiers you want to use, which point to the image per region.
  An example can be found in the modules directory.
