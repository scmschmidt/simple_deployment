...
output "machine_info" {
  value       = {
    for id, data in aws_instance.virtual_machine:
      "${data.tags["Name"]}" => {
        "id"         = data.id,
...
output "bastion_info" {
  value = {
    ...
    "public_ip_address"   = join("", aws_instance.bastion_virtual_machine[*].public_ip),
    "private_ip_address"  = join("", aws_instance.bastion_virtual_machine[*].private_ip)
  }
