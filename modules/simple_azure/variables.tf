...
variable "subnet" {
  description = "Subnet for Azure's virtual network and used for the IP addresses for the virtual machines."
  type        = string
...
variable "image" {
  description = "Identifier to select the correct source image for the virtual machine in the format '<publisher>:<offer>:<sku>'."
  type        = string
}

variable "size" {
  description = "Identifier to chose the size for the virtual machine."
  type        = string
}
